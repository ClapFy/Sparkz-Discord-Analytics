...
|----------|---------|----------------|
| `DISCORD_BOT_TOKEN` | bot; web recommended | Bot login. On the dashboard service, set the same token so widgets resolve **channels** (incl. threads via per-ID fetch), **users**, and **roles** via Discord REST (guild channel/role lists + fallbacks; raw IDs if unset). |
